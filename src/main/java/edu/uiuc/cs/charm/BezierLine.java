...
//      for (i=0;i<num;i++) q[i] = new kr.goldenmine.util.Point(p[i]);
        for (i=0;i<num;i++) q[i].copy(p[i]);
        for (i=1;i<=num-1;i++) {
//         r[num-i] = new kr.goldenmine.util.Point(q[num-1]);
            r[num-i].copy(q[num-1]);
            for (j=num-1;j>=i;j--) {
//	    q[j] = new kr.goldenmine.util.Point((q[j-1].x+q[j].x)/2, (q[j-1].y+q[j].y)/2);
                q[j].x = (q[j-1].x+q[j].x)/2;
                q[j].y = (q[j-1].y+q[j].y)/2;
            }
        }
//      r[0] = new kr.goldenmine.util.Point(q[num-1]);
        r[0].copy(q[num-1]);
    }
...
//	   qt = new kr.goldenmine.util.Point[num];
//	   rt = new kr.goldenmine.util.Point[num];
            qt = get_buf(num);
            rt = get_buf(num);
